...
| Ch | Title | Core concept | MERN/JS equivalent | Status | Implementing files | Notes |
|---|---|---|---|---|---|---|
| 4 | Agents 101 | Agent = LLM + memory + tools + goal; agent lifecycle; autonomy | Mastra `Agent` class with `model`, `instructions`, `tools`, `memory` | 📋 | *(Phase 2)* | Will use Embabel `@Agent` + `@Action` to replace direct ChatClient calls |
